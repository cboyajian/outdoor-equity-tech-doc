...
A state-level visitorshed map shows the number of visitors to overnight reservable sites from each state. We use the  package `zipcodeR` [@R-zipcodeR] to determine the state in which each ZIP code, and thus visitor, is located, using the RIDB data. We calculate the number of reservations per state for the site (as chosen by the app user) and use the `tmaps` package [@R-tmap] to create an interactive map that colors states based on the total number of reservations for that state. 
...
```{r, echo=FALSE}
# add diagram or table of shiny directory organization
```
